# Configure Variables
    ...
    # SQL Servers
    $BizTalkServer01 = $xml.Settings.General.BizTalk.BizTalkserver1
    $BizTalkServer02 = $xml.Settings.General.BizTalk.BizTalkserver2

    # Cluster Name
    $ClusterName = $xml.Settings.General.Cluster.ClusternameBizTalk
    $ClusterOU = $xml.Settings.General.Cluster.ClusterOU
 
    # IPs
    $ClusterIP = $xml.Settings.General.IPs.ClusterIPBizTalk
    $Subnetmask = $xml.Settings.General.IPs.Subnetmask
    $hb1 =$xml.Settings.General.IPs.HB1BizTalk
    $hb2 =$xml.Settings.General.IPs.HB2BizTalk
...
# Install Windows Features
    Install-WindowsFeature Failover-Clustering,NET-Framework-Core  -ComputerName $BizTalkServer01
    Install-WindowsFeature Failover-Clustering,NET-Framework-Core -ComputerName $BizTalkServer02


# Rename Heartbeat nic
    $c = New-CimSession -ComputerName $BizTalkServer01
    ...
    $c = New-CimSession -ComputerName $BizTalkServer02
    Rename-NetAdapter "Ethernet1" -newName "Heartbeat" -CimSession $c 
...
# Delegate permissions in AD
    ...
    New-Cluster -Name $ClusterName -Node "$BizTalkServer01","$BizTalkServer02" -StaticAddress $ClusterIP -NoStorage
   ...
   # Test-Cluster UUC-CLU002.exchange-test.its.uu.se -ReportName "c:\temp\Validate$ClusterName"  -Include 'Cluster Configuration','Inventory', 'Network', 'System Configuration' 
